...
var _require3 = require('./RelayConnectionInterface'),
...
var _require4 = require('./RelayContainerUtils'),
...
function createContainerWithFragments(Component, fragments, connectionConfig) {
  ...
  var Container = function (_React$Component) {
    ...
    Container.prototype._getConnectionData = function _getConnectionData() {
      ...
      var props = (0, _extends3['default'])({}, this.props, this.state.data);
      var connectionData = connectionConfig.getConnectionFromProps(props);
      ...
      var hasMore = connectionConfig.direction === FORWARD ? pageInfo[HAS_NEXT_PAGE] : pageInfo[HAS_PREV_PAGE];
      var cursor = connectionConfig.direction === FORWARD ? pageInfo[END_CURSOR] : pageInfo[START_CURSOR];
      if (typeof hasMore !== 'boolean' || typeof cursor !== 'string') {
        require('fbjs/lib/warning')(false, 'ReactRelayPaginationContainer: Cannot paginate without %s fields in `%s`. ' + 'Be sure to fetch %s (got `%s`) and %s (got `%s`).', PAGE_INFO, componentName, connectionConfig.direction === FORWARD ? HAS_NEXT_PAGE : HAS_NEXT_PAGE, hasMore, connectionConfig.direction === FORWARD ? END_CURSOR : START_CURSOR, cursor);
        return null;
      }
      return {
        cursor: cursor,
    ...
    Container.prototype._fetchPage = function _fetchPage(paginatingVariables, callback, options) {
      ...
      var pendingRefetch = environment.sendQuerySubscription({
    ...
    Container.prototype._updateSnapshots = function _updateSnapshots(totalCount) {
      ...
      var nextVariables = connectionConfig.getFragmentVariables(prevVariables, totalCount);
      ...
      // - Static core automatically appends new items, updating the count
