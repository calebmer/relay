...
var RelayPropTypes = {
  ...
  Environment: function Environment(props, propName, componentName) {
    ...
    if (!require('./isLegacyRelayEnvironment')(context) || !require('./isRelayEnvironment')(context)) {
  ...
  LegacyRelay: function LegacyRelay(props, propName, componentName) {
    var relay = props[propName];
    if (!require('./isRelayContext')(relay) || !require('./isLegacyRelayEnvironment')(relay.environment)) {
      return new Error(require('fbjs/lib/sprintf')('Invalid prop/context `%s` supplied to `%s`, expected `%s` to be ' + 'an object with a legacy `environment` implementation and `variables`.', propName, componentName, relay));
    }
    return null;
  },
