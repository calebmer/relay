...
var formatStorageKey = require('./RelayStoreUtils').formatStorageKey,
    getArgumentValues = require('./RelayStoreUtils').getArgumentValues,
    getStorageKey = require('./RelayStoreUtils').getStorageKey,
    TYPENAME_KEY = require('./RelayStoreUtils').TYPENAME_KEY;
...
var RelayResponseNormalizer = function () {
  ...
  RelayResponseNormalizer.prototype._getRecordType = function _getRecordType(data) {
    ...
    require('fbjs/lib/invariant')(typeName != null, 'RelayResponseNormalizer(): Expected a typename for record `%s`.', (0, _stringify2['default'])(data, null, 2));
    return typeName;
  };
  ...
  RelayResponseNormalizer.prototype._traverseSelections = function _traverseSelections(selections, record, data) {
    ...
    selections.forEach(function (selection) {
      if (selection.kind === SCALAR_FIELD || selection.kind === LINKED_FIELD) {
        ...
        var typeName = require('./RelayStaticRecord').getType(record);
        ...
        var fieldKey = formatStorageKey(selection.name, args);
        var handleKey = require('./getRelayHandleName')(selection.name, selection.handle);
        _this._handleFieldPayloads.push({
          args: args,
          dataID: require('./RelayStaticRecord').getDataID(record),
          fieldKey: fieldKey,
  ...
  RelayResponseNormalizer.prototype._normalizeField = function _normalizeField(selection, record, data) {
    ...
    if (fieldValue == null) {
      ...
      if (process.env.NODE_ENV !== 'production') {
        require('fbjs/lib/warning')(data.hasOwnProperty(responseKey), 'RelayResponseNormalizer(): Payload did not contain a value ' + 'for field `%s: %s`. Check that you are parsing with the same ' + 'query that was used to fetch the payload.', responseKey, storageKey);
      }
      require('./RelayStaticRecord').setValue(record, storageKey, null);
      return;
    }

    if (selection.kind === SCALAR_FIELD) {
      require('./RelayStaticRecord').setValue(record, storageKey, fieldValue);
  ...
  RelayResponseNormalizer.prototype._normalizeLink = function _normalizeLink(field, record, storageKey, fieldValue) {
    ...
    require('./RelayStaticRecord').getLinkedRecordID(record, storageKey) || require('./generateRelayClientID')(require('./RelayStaticRecord').getDataID(record), storageKey);
    require('fbjs/lib/invariant')(typeof nextID === 'string', 'RelayResponseNormalizer: Expected id on field `%s` to be a string.', storageKey);
    require('./RelayStaticRecord').setLinkedRecordID(record, storageKey, nextID);
    var nextRecord = this._recordSource.get(nextID);
    if (!nextRecord) {
      var typeName = field.concreteType || this._getRecordType(fieldValue);
      nextRecord = require('./RelayStaticRecord').create(nextID, typeName);
  ...
  RelayResponseNormalizer.prototype._normalizePluralLink = function _normalizePluralLink(field, record, storageKey, fieldValue) {
    ...
    var prevIDs = require('./RelayStaticRecord').getLinkedRecordIDs(record, storageKey);
    var nextIDs = [];
    fieldValue.forEach(function (item, nextIndex) {
      ...
      require('./generateRelayClientID')(require('./RelayStaticRecord').getDataID(record), storageKey, nextIndex);
    ...
    require('./RelayStaticRecord').setLinkedRecordIDs(record, storageKey, nextIDs);
  };
