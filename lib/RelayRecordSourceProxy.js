...
var RelayRecordSourceProxy = function () {
  ...
  RelayRecordSourceProxy.prototype.commitPayload = function commitPayload(selector, response) {
    ...
    var _normalizeRelayPayloa = require('./normalizeRelayPayload')(selector, response),
      ...
      if (status === EXISTENT) {
        ...
        if (sourceRecord) {
          if (_this._mutator.getStatus(dataID) !== EXISTENT) {
            _this.create(dataID, require('./RelayStaticRecord').getType(sourceRecord));
          }
  ...
  RelayRecordSourceProxy.prototype.get = function get(dataID) {
    if (!this._proxies.hasOwnProperty(dataID)) {
      ...
      if (status === EXISTENT) {
        this._proxies[dataID] = new (require('./RelayRecordProxy'))(this, this._mutator, dataID);
      } else {
