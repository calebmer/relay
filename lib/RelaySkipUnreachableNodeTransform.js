...
function transformNode(context, fragments, node) {
  var queue = [].concat(node.selections);
  ...
  while (queue.length) {
    ...
    if (selection.kind === 'Condition') {
      ...
      if (match === PASS) {
        queue.unshift.apply(queue, selection.selections);
      ...
      if (!fragments.has(selection.name)) {
        ...
        require('fbjs/lib/invariant')(fragment && fragment.kind === 'Fragment', 'RelaySkipUnreachableNodeTransform: Found a reference to undefined ' + 'fragment `%s`.', selection.name);
