...
var _require2 = require('./RelayConnectionInterface'),
    ...
    PAGE_INFO_TYPE = _require2.PAGE_INFO_TYPE,
    START_CURSOR = _require2.START_CURSOR;

var _require3 = require('./RelayNodeInterface'),
    ID = _require3.ID,
    NODE = _require3.NODE;
...
function update(proxy, payload) {
  ...
  if (!clientConnection) {
    ...
    var connection = proxy.create(require('./generateRelayClientID')(record.getDataID(), payload.handleKey), serverConnection.getType());
    ...
    clientPageInfo = proxy.create(require('./generateRelayClientID')(connection.getDataID(), PAGE_INFO), PAGE_INFO_TYPE);
 ...
 * ```
 */
function getConnection(record, fieldName, handleName) {
  var handleKey = require('./getRelayHandleName')(fieldName, handleName || CONNECTION);
  return record.getLinkedRecord(handleKey);
}
