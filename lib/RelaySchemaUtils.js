...
var _values2 = _interopRequireDefault(require('babel-runtime/core-js/object/values'));
...
var assertAbstractType = require('graphql').assertAbstractType,
    assertType = require('graphql').assertType,
...
function assertNamedType(type) {
  var namedType = getNamedType(assertType(type));
  require('fbjs/lib/invariant')(namedType === type, 'RelaySchemaUtils: Expected `%s` to be a named type.', type);
  return type;
}
 ...
 */
function schemaWithDirectives(schema, directives) {
  var combinedDirectives = [].concat(schema.getDirectives(), directives);
  // Validate uniquely named directives.
  var directiveNames = new _set2['default']();
  combinedDirectives.forEach(function (directive) {
    require('fbjs/lib/invariant')(!directiveNames.has(directive.name), 'RelaySchemaUtils: Expected unique names for directives, found a ' + 'duplicate directive `%s`.', directive.name);
    directiveNames.add(directive.name);
  });
  var types = (0, _values2['default'])(schema.getTypeMap()).map(assertNamedType);
...
function getOperationDefinitionAST(ast) {
...
function getSchemaDefinitionAST(ast) {
...
module.exports = {
  ...
  getOperationDefinitionAST: getOperationDefinitionAST,
  getRawType: getRawType,
  getSchemaDefinitionAST: getSchemaDefinitionAST,
  ...
  schemaWithDirectives: schemaWithDirectives
};