...
var RelayMarkSweepStore = function () {
  function RelayMarkSweepStore(source) {
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      for (var ii = 0; ii < storeIDs.length; ii++) {
        ...
        if (record) {
          require('./RelayStaticRecord').freeze(record);
        }
      }
    }
  ...
  RelayMarkSweepStore.prototype.resolve = function resolve(target, selector, callback) {
    require('./RelayAsyncLoader').load(this._recordSource, target, selector, callback);
  };
...
function updateTargetFromSource(target, source, updatedRecordIDs) {
  ...
  for (var ii = 0; ii < dataIDs.length; ii++) {
    ...
    if (process.env.NODE_ENV !== 'production') {
      if (sourceRecord) {
        require('./RelayStaticRecord').freeze(sourceRecord);
      }
    }
    if (sourceRecord === UNPUBLISH_RECORD_SENTINEL) {
      ...
      var nextRecord = require('./RelayStaticRecord').update(targetRecord, sourceRecord);
      if (nextRecord !== targetRecord) {
        // Prevent mutation of a record from outside the store.
        if (process.env.NODE_ENV !== 'production') {
          require('./RelayStaticRecord').freeze(nextRecord);
        }
        updatedRecordIDs[dataID] = true;
