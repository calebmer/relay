...
function getSelector(operationVariables, fragment, item) {
  require('fbjs/lib/invariant')(typeof item === 'object' && item !== null && !Array.isArray(item), 'RelayStaticSelector: Expected value for fragment `%s` to be an object, got ' + '`%s`.', fragment.name, (0, _stringify2['default'])(item));
  ...
  require('fbjs/lib/warning')(false, 'RelayStaticSelector: Expected object to contain data for fragment `%s`, got ' + '`%s`. Make sure that the parent operation/fragment included fragment ' + '`...%s`.', fragment.name, (0, _stringify2['default'])(item), fragment.name);
  return null;
}
...
function getSelectorsFromObject(operationVariables, fragments, object) {
  ...
  require('fbjs/lib/forEachObject')(fragments, function (fragment, key) {
    ...
    if (item == null) {
      ...
      require('fbjs/lib/invariant')(Array.isArray(item), 'RelayStaticSelector: Expected value for key `%s` to be an array, got `%s`. ' + 'Remove `@relay(plural: true)` from fragment `%s` to allow the prop to be an object.', key, (0, _stringify2['default'])(item), fragment.name);
      selectors[key] = getSelectorList(operationVariables, fragment, item);
    } else {
      require('fbjs/lib/invariant')(!Array.isArray(item), 'RelayStaticFragmentSpecResolver: Expected value for key `%s` to be an object, got `%s`. ' + 'Add `@relay(plural: true)` to fragment `%s` to allow the prop to be an array of items.', key, (0, _stringify2['default'])(item), fragment.name);
