...
var RelayReader = function () {
  ...
  RelayReader.prototype._readLink = function _readLink(field, record, data) {
    ...
    require('fbjs/lib/invariant')(prevData == null || typeof prevData === 'object', 'RelayReader(): Expected data for field `%s` on record `%s` ' + 'to be an object, got `%s`.', applicationName, require('./RelayStaticRecord').getDataID(record), prevData);
  ...
  RelayReader.prototype._readPluralLink = function _readPluralLink(field, record, data) {
    ...
    var prevData = data[applicationName];
    require('fbjs/lib/invariant')(prevData == null || Array.isArray(prevData), 'RelayReader(): Expected data for field `%s` on record `%s` ' + 'to be an array, got `%s`.', applicationName, require('./RelayStaticRecord').getDataID(record), prevData);
    ...
    linkedIDs.forEach(function (linkedID, nextIndex) {
      ...
      require('fbjs/lib/invariant')(prevItem == null || typeof prevItem === 'object', 'RelayReader(): Expected data for field `%s` on record `%s` ' + 'to be an object, got `%s`.', applicationName, require('./RelayStaticRecord').getDataID(record), prevItem);
  ...
  RelayReader.prototype._createFragmentPointer = function _createFragmentPointer(fragmentSpread, record, data) {
    ...
    data[ID_KEY] = data[ID_KEY] || require('./RelayStaticRecord').getDataID(record);
