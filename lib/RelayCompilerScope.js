...
function getFragmentScope(definitions, args, parentScope) {
  ...
  definitions.forEach(function (definition) {
    if (definition.kind === 'RootArgumentDefinition') {
      require('fbjs/lib/invariant')(!argMap.hasOwnProperty(definition.name), 'RelayCompilerScope: Unexpected argument for global variable `%s`. ' + '@arguments may only be provided for variables defined in the ' + 'fragment\'s @argumentDefinitions list.', definition.name);
      ...
      if (arg == null || arg.kind === 'Literal' && arg.value == null) {
        ...
        // value.
        require('fbjs/lib/invariant')(definition.defaultValue != null || !(definition.type instanceof GraphQLNonNull), 'RelayCompilerScope: No value found for required argument ' + '`$%s: %s`.', definition.name, definition.type.toString());
        fragmentScope[definition.name] = {
          kind: 'Literal',
