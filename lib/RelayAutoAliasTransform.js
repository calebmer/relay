...
function transformSelections(nodeSelections) {
  return nodeSelections.map(function (selection) {
    if (selection.kind === 'LinkedField') {
      ...
      require('fbjs/lib/invariant')(!selection.args.length, 'RelayAutoAliasTransform: Expected arguments to fragment spread ' + '`%s` to be inlined.', selection.name);
      return selection;
    } else {
...
function generateAlias(field) {
  ...
  var args = [].concat(field.args).sort(sortByName).map(function (arg) {
    return require('./getIdentifierForRelayArgumentValue')(arg.value);
  });
  var hash = require('./murmurHash')(require('./stableJSONStringify')(args));
