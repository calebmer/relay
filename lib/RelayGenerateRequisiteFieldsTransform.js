...
var TYPENAME_KEY = require('./RelayStoreUtils').TYPENAME_KEY;
...
function transformField(context, field) {
  ...
  var generatedSelections = [].concat(transformedNode.selections);
  var idSelections = generateIDSelections(context, field, field.type);
  if (idSelections) {
    generatedSelections.push.apply(generatedSelections, idSelections);
  }
 ...
 * @internal
 *
 * For interoperability with legacy systems, sort `__typename` first.
 */
function sortSelections(selections) {
  return [].concat(selections).sort(function (a, b) {
