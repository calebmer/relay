 ...
 * necessary static methods (`getFragment()` etc) to be composed within legacy
 * `Relay.Containers`.
 *
 ...
 */
function buildCompatContainer(ComponentClass, fragmentSpec, createContainerWithFragments, injectedVariableProvider) {
  ...
  function getDefaultVariables() {
    if (injectedDefaultVariables == null) {
      injectedDefaultVariables = injectedVariableProvider ? injectedVariableProvider() : {};
    }
  ...
  function getFragment(fragmentName, variableMapping) {
    ...
    var fragment = require('./RelayGraphQLTag').getLegacyFragment(taggedNode);

    // `injectedVariableProvider` allows legacy versions of a container to
    ...
    // in legacy queries that do not define the necessary param.
  ...
  function hasVariable(variableName) {
    return (0, _keys2['default'])(fragmentSpec).some(function (fragmentName) {
      var fragment = require('./RelayGraphQLTag').getLegacyFragment(fragmentSpec[fragmentName]);
  ...
  // Legacy container static methods
...
function assertFragmentMap(componentName, fragments) {
  require('fbjs/lib/forEachObject')(fragments, function (fragment, key) {
    require('fbjs/lib/invariant')(typeof fragment === 'object' && fragment !== null, 'ReactRelayCompatContainerBuilder: Could not create container for `%s`. The ' + 'value of fragment `%s` was expected to be a fragment, got `%s` instead.', componentName, key, fragment);
  });
}
...
module.exports = { buildCompatContainer: buildCompatContainer };