...
function visitField(field, state) {
  ...
  // Allow metadata fields and fields defined on legacy "fat" interfaces
  field.name === SchemaMetaFieldDef.name || field.name === TypeMetaFieldDef.name || field.name === TypeNameMetaFieldDef.name || field.directives.some(function (_ref) {
    ...
    require('fbjs/lib/invariant')(type, 'RelaySkipClientFieldTransform: Expected type `%s` to be defined in ' + 'the original schema.', rawType.name);
...
function visitFragmentSpread(spread, state) {
  ...
  require('fbjs/lib/invariant')(fragment && fragment.kind === 'Fragment', 'RelaySkipClientFieldTransform: Expected a fragment named `%s` to be defined.', spread.name);
