  ...
  if (node.kind === 'Root') {
    if (node.operation === 'mutation') {
      ...
      if (selection.kind === 'LinkedField') {
        ...
        var ir = require('./transformInputObjectToIR')(argument);
        return normalize(transform(ir)).map(function (n) {
  ...
  var findObjectTypeProperty = {
    enter: function enter(path) {
      if (require('babel-types').isObjectTypeProperty(path)) {
        ...
        path.traverse(findObjectTypeAnnotation, { name: this.prevName + '_' + name });
      }
    },
