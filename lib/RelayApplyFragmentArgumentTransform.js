...
function transformFragmentSpread(context, fragments, scope, spread) {
  ...
  require('fbjs/lib/invariant')(fragment && fragment.kind === 'Fragment', 'RelayApplyFragmentArgumentTransform: expected `%s` to be a fragment, ' + 'got `%s`.', spread.name, fragment && fragment.kind);
  ...
  selections.forEach(function (selection) {
    ...
    if (selection.kind === 'InlineFragment') {
      ...
      if (conditionSelections) {
        ...
        (_nextSelections = nextSelections).push.apply(_nextSelections, conditionSelections);
      }
    } else {
...
function hashArguments(args, scope) {
  ...
  var sortedArgs = [].concat(args).sort(function (a, b) {
