...
function getOperationVariables(operation, variables) {
  ...
  operation.query.argumentDefinitions.forEach(function (def) {
    ...
    if (process.env.NODE_ENV !== 'production') {
      require('fbjs/lib/warning')(value != null || !def.type.endsWith('!'), 'RelayConcreteVariables: Expected a value for non-nullable variable ' + '`$%s: %s` on operation `%s`, got `%s`. Make sure you supply a ' + 'value for all non-nullable arguments.', def.name, def.type, operation.name, (0, _stringify2['default'])(value));
    }
  });
