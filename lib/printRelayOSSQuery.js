...
function printRelayOSSQuery(node) {
  ...
  require('fbjs/lib/invariant')(queryText, 'printRelayOSSQuery(): Unsupported node type, got `%s`.', (0, _stringify2['default'])(node));
  var variables = {};
...
function printFragment(node, printerState) {
  ...
  return 'fragment ' + node.getDebugName() + ' on ' + node.getType() + directives + require('fbjs/lib/nullthrows')(printChildren(node, printerState, ''));
}
    ...
    if (child instanceof require('./RelayQuery').Field) {
      ...
      if (child.getChildren().length) {
        ...
        if (_fragmentNameByHash.hasOwnProperty(_fragmentHash)) {
          ...
          var fragmentChildren = require('fbjs/lib/nullthrows')(printChildren(child, printerState, ''));
