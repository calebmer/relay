...
var _require = require('./RelaySchemaUtils'),
    ...
    getTypeFromAST = _require.getTypeFromAST;

var assertAbstractType = require('graphql').assertAbstractType,
    assertCompositeType = require('graphql').assertCompositeType,
    assertInputType = require('graphql').assertInputType,
    assertLeafType = require('graphql').assertLeafType,
    assertOutputType = require('graphql').assertOutputType,
    isAbstractType = require('graphql').isAbstractType,
    isLeafType = require('graphql').isLeafType,
    GraphQLInputObjectType = require('graphql').GraphQLInputObjectType,
    ...
    isTypeSubTypeOf = require('graphql').isTypeSubTypeOf,
    parseType = require('graphql').parseType;

var SchemaMetaFieldDef = require('graphql/type/introspection').SchemaMetaFieldDef,
    TypeMetaFieldDef = require('graphql/type/introspection').TypeMetaFieldDef,
    TypeNameMetaFieldDef = require('graphql/type/introspection').TypeNameMetaFieldDef;
...
function parse(schema, text, filename) {
  var ast = require('graphql').parse(new Source(text, filename));
  var nodes = [];
  schema = require('graphql').extendSchema(schema, ast);
...
var RelayParser = function () {
  ...
  RelayParser.prototype._recordVariableReference = function _recordVariableReference(name, type) {
    ...
    if (type && prevType) {
      require('fbjs/lib/invariant')(this._referencedVariables[name] == null || isTypeSubTypeOf(this._schema, this._referencedVariables[name], type), 'RelayParser: Variable `$%s` was used in locations expecting ' + 'the conflicting types `%s` and `%s`. Source: %s.', getName(this._definition), prevType, type, this._getErrorContext());
    }
    ...
    if (argumentDirectives.length) {
      args = (argumentDirectives[0].arguments || []).map(function (arg) {
        return {
          ...
          metadata: null,
          name: getName(arg),
          value: _this5._transformValue(arg.value),
          type: null };
      });
    }
  ...
  RelayParser.prototype._transformField = function _transformField(field, parentType) {
    ...
    clientFieldDirectives.forEach(function (clientFieldDirective) {
      ...
      if (handleArgument) {
        ...
        require('fbjs/lib/invariant')(maybeHandle.kind === 'Literal' && typeof maybeHandle.value === 'string', 'RelayParser: Expected the %s argument to @%s to be a literal ' + 'string, got `%s` on field `%s`. Source: %s.', CLIENT_FIELD_HANDLE, CLIENT_FIELD, maybeHandle, name, _this6._getErrorContext());
        handles = handles || [];
        handles.push(maybeHandle.value);
      }
    });
    if (isLeafType(type)) {
...
function getFieldDefinition(schema, parentType, fieldName, fieldAST) {
  ...
  if (!schemaFieldDef) {
    schemaFieldDef = getLegacyFieldDefinition(schema, type, fieldName, fieldAST);
  }

  return schemaFieldDef || null;
}

function getLegacyFieldDefinition(schema, type, fieldName, fieldAST) {
